...
START_TEST (test_ettercap_http_bad_credentials) {
    ettercap_parser parser;
    ettercap_parser_init(&parser, HTTP_PORT);
    uint8_t  data[] = "GET /path HTTP/1.1\r\nAuthorization: Basic Fkhasdfnklajksjdflk\r\n\r\n";
    buffer b;
    FIXBUF(b, data);
    bool errored = false;
    ettercap_state state = ettercap_consume(&b, &parser, &errored);
    ck_assert_uint_eq(true, errored);
    ck_assert_uint_eq(ettercap_error, state);
    ck_assert_uint_eq(ettercap_error_http_bad_credential, parser.error);
...
ettercap_suite(void) {
    ...
    tcase_add_test(tc, test_ettercap_http_bad_credentials); 
