...
uint64_t b64_encoded_size(uint64_t inlen) {
    uint64_t ret;
...
uint64_t b64_decoded_size(const char * in) {
	uint64_t len;
	uint64_t ret;
	uint64_t i;
...
/* This code generated the table with magic numbers */
...
int32_t b64_isvalidchar(char c) {
	if (c >= '0' && c <= '9')
		return 1;
	if (c >= 'A' && c <= 'Z')
		return 1;
